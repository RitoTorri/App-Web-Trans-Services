...
import type { Nomina , Item} from "../types/models";
...
interface NominaEditarState {
    id: number | null;
    status: string;       // Cambiado de 'estado'
    employee_id: number;  
    period_start: string;   
    period_end: string;     
    daily_salary: number; 
...
function Nominas() {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // 2. Manejo de campos de fecha (¡Simplemente asignamos el string!)
        } else if (['period_start', 'period_end'].includes(name)) {
            // Si el input es type="date", el 'value' ya es el string "YYYY-MM-DD" que necesitamos.
    ...
    return;
        ...
        const filterBody = terminoBusqueda ? { filterSearch: { status: terminoBusqueda } } : {};  // Ejemplo: filtra por status si hay búsqueda
    ...
    const registrosParseados: Nomina[] = data.details.map((item: any) => {
        ...
        // Función auxiliar para obtener 'YYYY-MM-DD' de una cadena ISO o vacía
        const extractDateString = (dateInput: string | undefined): string => {
            if (!dateInput) return '';
            // Separa la fecha (parte antes de 'T')
        ...
        return {
            ...
            // ⭐ CAMBIO CLAVE: Extraer solo la parte 'YYYY-MM-DD' como string
            period_start: extractDateString(apiPeriodStart),
            period_end: extractDateString(apiPeriodEnd),
...
//Registrar Nominas
    ...
    try {
            const dataToSend = {
                employee_id: state.form.employee_id,
                period_start: state.form.period_start,
                period_end: state.form.period_end,
                daily_salary: state.form.daily_salary,
                total_days_paid: state.form.total_days_paid,
                ivss: state.form.ivss,  
                pie: state.form.pie,    
                faov: state.form.faov,  
            };
            ...
            <form id="FormularioEditarNomina" onSubmit={manejadorSubmitEditar} className="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input
                        ...
                        name="period_start"
                        onChange={handleInputChangeEdit}
                        value={nominaEditar.period_start || ''} 
                        className="border border-gray-400 rounded-md mb-2 shadow-xs w-full p-3 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:border-gray-300 transition-all ease-in"
                    />
                </div>
                <div>
