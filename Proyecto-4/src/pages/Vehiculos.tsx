...
/// 1. Preparar datos para la API (Renombrar drive_id a driver_id)
    const payload = {
        ...(datosEditar.drive_id && { driver_id: Number(datosEditar.drive_id) }), // Renombramos si existe
  ...
  <select
    name="drive_id" 
    // Usamos drive_id que es como se llama en tu modelo Vehiculo
  ...
  <select
    name="drive_id"
    // Usamos vehiculoEditar.drive_id
